// <auto-generated />
using System;
using DataFromExcel.Infrastructure.Repositories;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace DataFromExcel.Infrastructure.Repositories.MSSql.Migrations
{
    [DbContext(typeof(ObjectOfSaleContext))]
    [Migration("20260813063234_AddGeneralContractorMarkup")]
    partial class AddGeneralContractorMarkup
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.6")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("DataFromExcel.Domain.AreaOfActivityPayment", b =>
                {
                    b.Property<Guid>("RowId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("AreaOfActivity")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContractIdIncome")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("DirectOrIndirect")
                        .HasColumnType("bit");

                    b.Property<string>("DocumentId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("Percent")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("TypeOfActivity")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("RowId");

                    b.ToTable("AreaOfActivityPayments");
                });

            modelBuilder.Entity("DataFromExcel.Domain.ObjectOfSaleInContract", b =>
                {
                    b.Property<string>("ContractId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("ComplexProperty")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CostItem")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("GeneralContractorMarkup")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Property")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("ContractId");

                    b.ToTable("ObjectOfSaleInContracts");
                });

            modelBuilder.Entity("DataFromExcel.Domain.ObjectOfSaleInPurchasePayment", b =>
                {
                    b.Property<string>("DocumentId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ComplexProperty")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ContractId")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("CostItem")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Property")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("DocumentId");

                    b.ToTable("ObjectOfSaleInPurchasePayments");
                });

            modelBuilder.Entity("DataFromExcel.Domain.TotalFloorArea", b =>
                {
                    b.Property<Guid>("RowId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("ApartmentArea")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Property")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("TotalArea")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("СommercialArea")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("RowId");

                    b.ToTable("TotalFloorAreas");
                });
#pragma warning restore 612, 618
        }
    }
}
